  ...
  <div class="col-lg-12">
    <div class="panel panel-default">
      ...
      <div class="panel-body">
        ...
        <!-- Page714 CSRF Token?? hidden input?ºÎ°ú Ï∂îÍ??? -->
	...
	$(document).ready(function(e){
		...
		$("button[type='submit']").on("click", function(e){
			...
			$(".uploadResult ul li").each(function(i, obj){
				...
				// file?? ?ÑÏÜ°?? ?åÏóê type?¥ÎÇò uuid?Ä Í∞ôÏ? ?ïÎ≥¥Î•? ?®Íªò ?ÑÎã¨?òÍ∏∞ ?ÑÌïú
				// hidden input tagÎ•? Ï∂îÍ?
		...
		// Î≥¥Ïïà?ÅÏùò ?¥Ïú†Î°? Ï≤®Î??åÏùº?? ?ïÏû•?êÎ? ?úÌïú
		...
		function checkExtension(fileName, fileSize) {
			...
			if(regex.test(fileName)){
				// RegEx(?ïÍ∑ú?úÌòÑ??)?ºÎ°ú file?? ?¥Î¶Ñ?? Í≤ÄÏ¶?
				alert("Invalid type");
				return false;
			}
			return true;
		...
		// Page 721 : CSRF token?? Header?? ?ÑÎã¨?òÍ∏∞ ?ÑÌïò?? Î≥Ä?òÏÑ†??. ajax
		// ?êÏÑú data ?ÑÎã¨ ?? tokenÍ≥? headername?? ?®Íªò ?ÑÎã¨?òÍ≤å ?úÎã§
		...
		$("input[type='file']").change(function(e){
			...
			$.ajax({
				...
				// csrf token?? data ?ÑÏÜ° ?ÑÏóê headerÎ°? ?ÑÏÜ°
				data: formData,
				type: 'POST',
				dataType:'json',
		...
		function showUploadResult(uploadResultArr){
			...
			$(uploadResultArr).each(function(i, obj){
				...
				if(obj.image){
					...
					// str += "<li><div>";
					// Page563 : Ï≤®Î? file ?ïÎ≥¥Î•? tagÎ°? ?®Íªò ?ÑÎã¨
		...
		$(".uploadResult").on("click", "button", function(e){
			...
			$.ajax({
				...
				success: function(result){
					...
					targetLi.remove();
					// ListItem?? ??†ú?òÏó¨ ?ÖÎ°ú?úÌïú file?? Î≥¥Ïù¥ÏßÄ ?äÎèÑÎ°? ??
				}
			}); //$.ajax
   ...
   $(document).ready(function(e){
      ...
      function checkExtension(fileName, fileSize) {
         if(fileSize >= (maxSize * 4)) { // Up to 20MB
            alert("?ÖÎ°ú?? ?åÏùº?Ä 20MBÎ•? Ï¥àÍ≥º?? ?? ?ÜÏäµ?àÎã§");
            return false;
         }
         if(!regex.test(fileName)){
            // RegEx(?ïÍ∑ú?úÌòÑ??)?ºÎ°ú file?? ?¥Î¶Ñ?? Í≤ÄÏ¶?
            alert("?¨Î∞îÎ•¥Ï? ?äÏ? ?†Ìòï?? ?åÏùº?ÖÎãà??");
            return false;
         }
         return true;
      };
